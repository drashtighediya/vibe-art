...
$debugInfo = '';
if (isset($_GET['search']) || isset($_GET['category']) || isset($_GET['min_price']) || isset($_GET['max_price'])) {
    $debugInfo .= "<div style='background: #e3f2fd; padding: 15px; border-radius: 8px; border: 2px solid #2196F3; margin-bottom: 20px;'>";
    $debugInfo .= "<h3 style='margin-top: 0;'>🔍 Active Filters:</h3>";
    $debugInfo .= "<p><strong>Search:</strong> " . htmlspecialchars($search) . " (Length: " . strlen($search) . ")</p>";
    $debugInfo .= "<p><strong>Category:</strong> " . htmlspecialchars($category) . "</p>";
    $debugInfo .= "<p><strong>Min Price:</strong> " . ($min_price !== null ? $min_price : 'Not set') . "</p>";
    $debugInfo .= "<p><strong>Max Price:</strong> " . ($max_price !== null ? $max_price : 'Not set') . "</p>";
    $debugInfo .= "</div>";
}
...
$debugInfo .= "<div style='background: #fff3cd; padding: 15px; border-radius: 8px; border: 2px solid #ffc107; margin-bottom: 20px;'>";
$debugInfo .= "<h3 style='margin-top: 0;'>📊 Query Results:</h3>";
$debugInfo .= "<p><strong>Total Artworks Found:</strong> " . $artworks->num_rows . "</p>";
$debugInfo .= "<p><strong>Has Title Column:</strong> " . ($hasTitle ? 'Yes' : 'No') . "</p>";
$debugInfo .= "<p><strong>Has Description Column:</strong> " . ($hasDescription ? 'Yes' : 'No') . "</p>";
$debugInfo .= "<p><strong>Has Price Column:</strong> " . ($hasPrice ? 'Yes' : 'No') . "</p>";
$debugInfo .= "<p><strong>Has Image Path Column:</strong> " . ($hasImagePath ? 'Yes' : 'No') . "</p>";
$debugInfo .= "<p><strong>Artist Table Exists:</strong> " . ($artistTableExists ? 'Yes' : 'No') . "</p>";
$debugInfo .= "<p><strong>Category Table Exists:</strong> " . ($categoryTableExists ? 'Yes' : 'No') . "</p>";
$debugInfo .= "<p><strong>Final SQL Query:</strong></p>";
$debugInfo .= "<pre style='background: white; padding: 10px; border-radius: 5px; overflow-x: auto; font-size: 12px;'>" . htmlspecialchars($query) . "</pre>";
$debugInfo .= "<p><strong>Bound Parameters:</strong> " . json_encode($params) . "</p>";
$debugInfo .= "</div>";

$stmt->close();
$debugMode = isset($_GET['debug']) ? true : false;
if ($debugMode) {
    echo "<div style='background: #fff3cd; padding: 15px; margin: 20px; border-radius: 8px; border: 2px solid #ffc107;'>";
    echo "<h3>Debug Information:</h3>";
    echo "<p><strong>Search Term:</strong> " . htmlspecialchars($search) . "</p>";
    echo "<p><strong>Category:</strong> " . htmlspecialchars($category) . "</p>";
    echo "<p><strong>Min Price:</strong> " . $min_price . "</p>";
    echo "<p><strong>Max Price:</strong> " . $max_price . "</p>";
    echo "<p><strong>SQL Query:</strong> " . htmlspecialchars($query) . "</p>";
    echo "<p><strong>Total Results:</strong> " . $artworks->num_rows . "</p>";
    echo "</div>";
}
    ...
    <div class="center-content" style="<?php echo !$user_id ? 'margin-left: 0;' : ''; ?>">
        ...
        <div class="artworks-container">
            ...
            <div class="results-count">
                Found <?php echo $artworks->num_rows; ?> artwork(s)
            </div>
                            ...
                            <div class="artwork-image-container">
                                <?php if ($hasImagePath && !empty($artwork['image_path'])): ?>
                                    ...
                                    <img src="<?php echo htmlspecialchars($imagePath); ?>" 
                                         ...
                                         class="artwork-image"
                                         onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\'artwork-image\' style=\'display: flex; align-items: center; justify-content: center; color: white; font-size: 18px; flex-direction: column;\'><div style=\'font-size: 40px; margin-bottom: 10px;\'>🖼️</div><div>Image Not Found</div><div style=\'font-size: 12px; margin-top: 5px; opacity: 0.8;\'><?php echo htmlspecialchars($artwork['image_path']); ?></div></div>';">
                                <?php else: ?>
